...
(function() {
  ...
  function attachHover() {
    document.querySelectorAll('a, button, [role="button"], .work-card, .service-card, .value-card, .pricing-card, .service-full-card, .team-card, .stat, .timeline-item, .contact-detail').forEach(el => {
  ...
  attachHover();
  // Re-attach after dynamic content (portfolio)
...
(function() {
  ...
  // Re-observe after stagger adds classes
...
(function() {
  if (window.matchMedia('(pointer: coarse)').matches) return;
  document.querySelectorAll('.service-card, .pricing-card, .value-card, .work-card, .service-full-card').forEach(card => {
...
// ── 10. FORCE-SHOW IN-VIEWPORT REVEALS ───
// Catches elements already on screen when page loads (above the fold)
(function() {
  ...
  // Run immediately and again after a short delay (for stagger-generated children)
  forceVisible();
  setTimeout(forceVisible, 100);
  setTimeout(forceVisible, 400);
})();
